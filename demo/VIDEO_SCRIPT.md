...
then the catch-rate statistic (Beat 2 in the app + the engineer-behavior figure).
...
hypothesis it proposed: of its coherently-argued ideas, [FILL catch-rate]% failed held-out
validation, and the gate caught all of them. An AI scientist that knows when it's wrong — the
thing the field is most afraid it can't do."
...
Fill `[FILL catch-rate]` from `engineer_behavior.json` (Work Stream A). Framing rule: no
prediction win, no disease discovery; STK11 is shown proposed-and-rejected, and you say so.
